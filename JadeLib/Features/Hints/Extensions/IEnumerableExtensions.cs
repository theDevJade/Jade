...
public static class IEnumerableExtensions
{
    ...
    /// <returns>
    ///     true if there is only one element in the <see cref="IEnumerable{T}" /> and that element passes the filter,
    ///     otherwise false.
    /// </returns>
    ...
    public static IEnumerable<TValue> GetMultiple<TKey, TValue>(
        ...
        TKey key)
    {
        return dict.TryGetValue(key, out var value) ? value : Enumerable.Empty<TValue>();
    }
}